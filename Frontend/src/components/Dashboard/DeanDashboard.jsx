...
const DeanDashboard = () => {
  ...
  const handleDecision = async (applicationId, decision) => {
    ...
    const result = await DeanDecision(applicationId, decision);

    if (result) {
      SuccessToast(
        decision === "Approved"
          ? "Application finalized successfully"
  ...
  return (
    <div className="container mt-4">
      <h4 className="mb-3">Dean Panel</h4>
      <p className="text-muted">Final approval for admissions</p>
            ...
            <tbody>
              {applications.map((app, index) => (
                <tr key={app._id}>
                  ...
                  <td>{app.department?.UserEmail || "-"}</td>
                  <td>{app.mobile}</td>
